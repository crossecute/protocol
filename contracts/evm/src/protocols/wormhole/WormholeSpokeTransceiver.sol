...
///      — no provider chain id here, unlike Wormhole's own `uint16`.
contract WormholeSpokeTransceiver is SpokeTransceiverBase {
    ...
    ///      uses `WormholeZkSyncSpokeTransceiver`/`WormholeTronSpokeTransceiver` instead.
    function initialize(
    ...
    /// @notice No gateway granted yet. See `WormholeHubTransceiver` for why no R3.3
    ///         exception is needed.
}
