...
///
/// @dev THE ROUTE IS THE CHAIN IDENTIFIER, NOT HYPERLANE'S `uint32` DOMAIN FOR IT, which makes
...
contract HyperlaneSpokeTransceiver is SpokeTransceiverBase {
    ...
    ///      diverges needs `HyperlaneZkSyncSpokeTransceiver` or `HyperlaneTronSpokeTransceiver`
    ///      instead.
    function initialize(
    ...
    ///      quietly on a forged one. See `HyperlaneHubTransceiver` for why no R3.3-style
    ///      written exception is needed for the authentication order here either, provided
    ///      `Router` is skipped.
