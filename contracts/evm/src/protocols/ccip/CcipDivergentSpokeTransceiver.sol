...
///      independently and wrongly. CCIP's own chain-selector table is exactly as orthogonal
///      to this as LayerZero's eid table was: whichever provider carries the message, zkSync
///      and Tron still need their own address arithmetic.
...
contract CcipTronSpokeTransceiver is TronSpokeTransceiver {
    ...
    ///        is not solc's. See `CcipZkSyncSpokeTransceiver` for why it is an argument.
    function initialize(
        address[] calldata gateways,
        address receiverImplementation_,
        bytes32 homeChainKey_,
