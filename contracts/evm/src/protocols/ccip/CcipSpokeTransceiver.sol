...
///
/// @dev THE ROUTE IS THE CHAIN IDENTIFIER, NOT CCIP'S `uint64` SELECTOR FOR IT, which makes
...
contract CcipSpokeTransceiver is SpokeTransceiverBase {
    ...
    ///      diverges needs `CcipZkSyncSpokeTransceiver` or `CcipTronSpokeTransceiver` instead.
    function initialize(
    ...
    ///      quietly on a forged one. See `CcipHubTransceiver` for why no R3.3-style written
    ///      exception is needed for the authentication order here either.
